...
function Main() {
	...
	const isAuthenticated = () => {
		axios
			.get(`${process.env.REACT_APP_API_URL}/users/auth`, { withCredentials: true })
			.then((response) => {
		...
		<div>
			...
			<Wrapper>
              ...
              6.877c-3.793 0-6.877-3.085-6.877-6.877z"
				...
				{/* 글쓰기 버튼 */}
				<WritingButton openPost={openPost} onClick={openModalPostingWrite}>
					글쓰기
				</WritingButton>
				{/* 채팅 버튼 */}
				<ChattingButton openPost={openPost} onClick={openModalChatting}>
					채팅
				</ChattingButton>
				{post ? (
					<MobileButton openPost={openPost} onClick={openPostList}>
						배달 목록 {post.length}개{" "}
					</MobileButton>
				) : (
					<MobileButton openPost={openPost} onClick={openPostList}>
			...
			{/* 글쓰기 Modal */}
			{PostingWriteModal === true ? (
				<PostingWrite
					handleWritingAddress={handleWritingAddress}
					PostingWriteModal={PostingWriteModal}
					openModalPostingWrite={openModalPostingWrite}
				></PostingWrite>
			) : null}

			{/* 채팅 Modal */}
			{ChattingModal === true ? <Chatting setChattingModal={setChattingModal}></Chatting> : null}
		</div>
	);
}
